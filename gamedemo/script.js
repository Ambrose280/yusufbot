...
var maxSpeed = 48;
var monsterPos = .65;
var monsterPosTarget = .9;
var floorRotation = 0;
var collisionObstacle = 10;
var collisionBonus = 20;
var gameStatus = "play";
var cameraPosGame = 199;
var cameraPosGameOver = 260;
// var monsterAcceleration = 0.00004;
var monsterAcceleration = 0.005;
...
const number = parseInt(queryString, 10);

// console.log(number);

function sendRequest(gamepts) {
  const myHeaders = new Headers();
  myHeaders.append("Authorization", "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyMSIsImV4cCI6NzExOTgyOTU1M30.kFvro8Ox6l2ElpM8vqhVrkU8dx5U3vqdZ9RSEiiu0aw");

  const formdata = new FormData();
  formdata.append("gamepts", gamepts);
  ...
  fetch("http://127.0.0.1:8000/user/" + number + "/gamepts", requestOptions)
...
// const zuzu = param.split('?')[1];

// const no = parseInt(zuzu, 10);
...
function gameOver() {
  ...
  // Get the value from div with id "distValue"
  var distValue = parseInt(document.getElementById("distValue").innerText);
  sendRequest(distValue)
  getGamePts()
...
document.getElementById('start-button').addEventListener('click', init, false);
function init(event){
  initScreenAnd3D();
  createLights();
  createFloor();
